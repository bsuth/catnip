...
#include "server/lua_api/utils.h"
#include "server/user_config/keybindings.h"
#include <glib.h>
#include <lauxlib.h>
#include <lua.h>
#include <string.h>
...
lib_keybindings_add(lua_State* L)
{
  ...
  for (int i = 1; i < num_args - 1; ++i) {
    if (lua_type(L, i) != LUA_TSTRING) {
      g_warning("%s", get_arg_type_error_msg(L, i, LUA_TSTRING));
      return 0;
    }
    ...
    if (modifier == 0) {
      g_warning("%s", get_arg_error_msg(L, i, "unknown modifier"));
      return 0;
    }
  ...
  if (keysym == XKB_KEY_NoSymbol) {
    g_warning("%s", get_arg_error_msg(L, -2, "unknown key"));
    return 0;
  }
...
lib_keybindings_remove(lua_State* L)
{
  ...
  if (lua_type(L, -1) != LUA_TSTRING) {
    g_warning("%s", get_arg_type_error_msg(L, -1, LUA_TSTRING));
    return 0;
  }
  ...
  if (keysym == XKB_KEY_NoSymbol) {
    g_warning("%s", get_arg_error_msg(L, -1, "unknown key"));
    return 0;
  }
