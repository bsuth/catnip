...
on_window_unmap(struct wl_listener* listener, void* data)
{
  ...
  // visible on screen.
  catnip_lua_window_destroy(catnip_L, window->lua_window);
}
...
on_window_request_maximize(struct wl_listener* listener, void* data)
{
  ...
  catnip_lua_window_publish(catnip_L, window->lua_window, "maximize", 0);
...
on_window_request_fullscreen(struct wl_listener* listener, void* data)
{
  ...
  catnip_lua_window_publish(catnip_L, window->lua_window, "fullscreen", 0);
